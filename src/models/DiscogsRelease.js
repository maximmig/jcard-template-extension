...
export default class DiscogsRelease extends Release {
  ...
  initFromDocument() {
    ...
    const imagesEntryKey = Object.keys(release).find(k => k.startsWith('images'));
    const images = imagesEntryKey ? release[imagesEntryKey] : null;
    if (images) {
      const imageNodeKey = images.edges.length > 0 ? images.edges[0].node.__ref : null;
      if (imageNodeKey) {
    ...
    const trackNames = tracks
      .map(({__ref}) => {
        ...
        return (track.trackType === 'TRACK' && track.position !== 'Video') ? track.title : null;
      })
      .filter(t => t !== null);
